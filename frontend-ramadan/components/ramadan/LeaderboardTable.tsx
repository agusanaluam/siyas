    ...
    <div className="space-y-4">
        ...
        <div className="flex items-end justify-center gap-3 py-4">
          ...
          {top3.length > 1 && (
            <div className="flex flex-col items-center">
              ...
              <p className={`text-xs font-semibold text-center max-w-[80px] truncate ${
                top3[1].is_current_user ? 'text-ramadan-sage-dark' : 'text-ramadan-brown dark:text-white'
              }`}>
                {top3[1].name}
              </p>
              <p className="text-xs text-ramadan-brown/50 dark:text-gray-400">{top3[1].total_points} poin</p>
          ...
          {top3.length > 0 && (
            <div className="flex flex-col items-center -mt-4">
              ...
              <p className={`text-sm font-bold text-center max-w-[80px] truncate ${
                top3[0].is_current_user ? 'text-ramadan-sage-dark' : 'text-ramadan-brown dark:text-white'
              }`}>
                {top3[0].name}
              </p>
              <p className="text-xs text-ramadan-brown/50 dark:text-gray-400">{top3[0].total_points} poin</p>
          ...
          {top3.length > 2 && (
            <div className="flex flex-col items-center">
              ...
              <p className={`text-xs font-semibold text-center max-w-[80px] truncate ${
                top3[2].is_current_user ? 'text-ramadan-sage-dark' : 'text-ramadan-brown dark:text-white'
              }`}>
                {top3[2].name}
              </p>
              <p className="text-xs text-ramadan-brown/50 dark:text-gray-400">{top3[2].total_points} poin</p>
        ...
        {rest.map((entry) => (
          <div
            ...
            className={`card flex items-center gap-3 ${
              entry.is_current_user ? 'ring-2 ring-ramadan-sage bg-ramadan-sage/5' : ''
            }`}
          >
            <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
              <span className="text-sm font-bold text-ramadan-brown/60 dark:text-gray-400">
                {entry.rank}
              </span>
            </div>
            <div className="flex-1 min-w-0">
              <p className={`font-semibold text-sm truncate ${
                entry.is_current_user ? 'text-ramadan-sage-dark' : 'text-ramadan-brown dark:text-white'
              }`}>
                {entry.name}
                {entry.is_current_user && (
                  <span className="text-xs ml-1 text-ramadan-sage">(Kamu)</span>
                )}
              </p>
            </div>
            <div className="text-right flex-shrink-0">
              <p className="font-bold text-sm text-ramadan-brown dark:text-white">{entry.total_points}</p>
              <p className="text-xs text-ramadan-brown/40 dark:text-gray-500">poin</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {entries.length === 0 && (
        <div className="card text-center py-8">
          <p className="text-ramadan-brown/50 dark:text-gray-400">Belum ada data peringkat</p>
        </div>
      )}
    </div>
  )
