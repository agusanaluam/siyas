$(document).ready(function() {
    ...
    $('#group-save').on('click', function(e) {
        e.preventDefault();
        save();
    });
...
function edit(id) {
        $.ajax({
            url: `${BASE_URL}`+`/volunteer/group/`+id,
            type: "GET",
            cache: false,
            data: {
                    '_method': "GET",
...
function update() {
        ...
        const formData = new FormData($('#edit-group-form')[0]);

        // Memanggil AJAX
        var url = `${BASE_URL}`+'/volunteer/group/update';
...
function save() {
    ...
    const formData = new FormData($('#add-group-form')[0]);

        // Memanggil AJAX
        var url = `${BASE_URL}`+'/volunteer/group/store';
