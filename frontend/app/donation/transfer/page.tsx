...
export default function DonationTransferPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && (user?.level === 'administrator' || user?.level === 'root')) {
      fetchTransferDonations()
    }
  }, [isAuthenticated, user])

  const fetchTransferDonations = async () => {
    try {
      setDataLoading(true)
      const data = await donationService.getTransfer()
      setDonations(data)
    } catch (error) {
      console.error('Error fetching transfer donations:', error)
    } finally {
    ...
    <DashboardLayout>
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Donation Transfer</h1>
            <p className="mt-1 text-sm text-gray-600">Donation via transfer yang perlu approval</p>
          </div>
